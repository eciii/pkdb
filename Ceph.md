...
Ceph provides three _high-level storage service libraries_ already built on top of librados:

- RGW for object storage
- RBD for block storage
- CephFS for file storage

In practice applications never use librados directly. They use instead one (or more) of the three high-level storage service libraries provided by Ceph.

## The RADOS cluster

The RADOS cluster is the foundational layer on which Ceph is built. As a cluster, it is composed of 3 types of nodes (implemented as processes):
...
- Each replica is fully stored in an OSD. The OSDs are chosen (pseudo)randomly according to the pool's placement policy.
